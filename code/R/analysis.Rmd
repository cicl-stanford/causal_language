...
ggplot(df_to_show, 
       mapping = aes(x = response,
                     fill = verb)) +
  geom_bar(mapping = aes(linewidth = is_med),
           color="black",
           alpha = 0.6) +
  ...
  scale_fill_manual(values = c("hotpink", "mediumpurple2", "dodgerblue1")) +
  scale_color_manual(values = c("hotpink", "mediumpurple2", "dodgerblue1")) +
  facet_wrap(~verb,
             ncol = 3) +
...
df_boots = df_trials %>%
  ...
  mutate(comp = case_when(verb_ord == "AE" | verb_ord == "EA" ~ "affected-enabled",
         ...
         type = factor(type, levels = c("non-contradictory", "contradictory")),
         response = response + 1)
...
df_counts = df_to_show %>% 
  group_by(comp, type, response) %>% 
  summarise(total = n()) %>% 
  mutate(x_position = response + ifelse(type == "contradictory",
                                        dodge_width/2,
                                        -dodge_width/2))
...
df_text = df_to_show %>% 
  select(comp,
         type) %>% 
  unique() %>% 
  mutate(x_pos = ifelse(type == "non-contradictory", 2.4, 5.6)) %>% 
  arrange(comp, type)

df_text["label"] = c("affected \u2192 caused",
                     "caused \u2192 affected",
                     "affected \u2192 enabled",
                     "enabled \u2192 affected",
                     "enabled \u2192 caused",
...
ggplot(df_to_show,
  ...
  geom_bar(mapping = aes(x = x_position,
                         y = total,
                         linewidth = is_med),
           stat = "identity",
           color = "black",
...
df_trials = df_trials %>% 
  mutate(verb_ord = factor(toupper(paste(substr(verb1, 1, 1),
                                         ...
                                         sep="")),
                           levels = verb_ord_levels)) %>% 
  select(workerid, item_num, item, order, verb_ord, verb1, verb2, response) %>% 
...
df_boots = df_trials %>%
  ...
  mutate(comp = case_when(verb_ord == "AE" | verb_ord == "EA" ~ "affected-enabled",
         ...
         type = factor(type, levels = c("implicature", "implication")),
         response = response + 1)
```
...
df_counts = df_to_show %>% 
  group_by(type, comp, response) %>% 
  summarise(total = n()) %>% 
  mutate(x_position = response + ifelse(type == "implicature",
                                        -dodge_width/2,
                                        dodge_width/2))
...
df_text = df_to_show %>% 
  select(comp,
         type) %>% 
  unique() %>% 
  mutate(x_pos = ifelse(type == "implicature", 2.4, 5.6)) %>% 
  arrange(comp, x_pos)
...
ggplot(df_to_show,
  ...
  coord_cartesian(clip = "off") +
  scale_x_continuous(breaks = seq(1,7)) +
  scale_fill_manual(values = c("darkorchid1", "springgreen3")) +
  scale_color_manual(values = c("darkorchid1", "springgreen3")) +
  scale_linewidth_manual(values = c(0.5, 1.2)) +
  xlab("Response") +
  ylab("Count") +
...
ggsave("figures/listener_scatter.jpg",
       width = 15,
       height = 5)
...
df_cv_split = df_cv %>% 
  filter(split == 4) %>% 
  mutate(is_33 = trial == 33)

ggplot(df_cv_split,
       mapping = aes(x = model_pred,
                     y = data_val)) +
  geom_point(mapping = aes(color = is_33)) +
  geom_smooth(method = "lm") + 
...
df_cv_perf_wide = df_cv_perf %>%
  pivot_wider(id_cols = c(model, split),
              names_from = c(use),
              values_from = c(split_r, tr33_in_block)) %>% 
  mutate(test_better = split_r_test > split_r_train) 

df_cv_perf_wide %>% 
  group_by(model) %>% 
  summarise(tr33_predicts_better = sum(tr33_in_block_train == test_better),
            tr33_in_train = sum(tr33_in_block_train),
