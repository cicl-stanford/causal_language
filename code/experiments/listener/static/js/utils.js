...
// of trials.
function update_progress(num, num_trials, segment) {
    debug("update progress");
    var width = 2 + 98 * (num / (num_trials - 1.0));

    indicator_string = "indicator-stage"
    progress_string = "progress-text"

    if (segment == "train") {
        indicator_string = "training-" + indicator_string
        progress_string = "training-" + progress_string
    }

    $("#" + indicator_string).css({"width": width + "%"});
    $("#" + progress_string).html("Progress " + (num + 1) + "/" + num_trials);
